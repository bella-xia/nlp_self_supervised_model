...
class NPLMFirstBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        input_embeds = input_embeds.view(-1, self.local_window_size * self.embed_dim)


        # Transform embeddings with a linear layer and tanh activation
        feed_forward = torch.tanh(self.linear(input_embeds))


        # apply layer normalization
        normed_ff = self.layer_norm(feed_forward)


        # apply dropout
        final_embeds = self.dropout(normed_ff)
...
class NPLMBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        feed_forward = torch.tanh(self.layer_norm(inputs))

        # add residual connection
        added_res = feed_forward + inputs


        # apply layer normalization
        normed_ff = self.layer_norm(added_res)


        # apply dropout
        final_inputs = self.dropout(normed_ff)
...
class NPLMFinalBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        feed_forward = self.linear(inputs)

        # apply log_softmax to get log-probabilities (logits)
        log_probs = nn.functional.log_softmax(feed_forward)
...
class NPLM(nn.Module):
    def __init__(self, vocab_size, embed_dim, local_window_size, hidden_dim, num_blocks, dropout_p):
        ...
        for i in range(num_blocks):
            self.intermediate_layers.add_module(f"{i}", NPLMBlock(hidden_dim, dropout_p))
        #raise NotImplementedError
    ...
    def forward(self, inputs):
        ...
        #raise NotImplementedError
        # input layer
        feed_forward = self.first_layer(inputs)
        ...
        # remember to apply the ReLU activation function after each layer
            feed_forward = nn.functional.relu(feed_forward)

        # output layer
        log_probs = self.final_layer(feed_forward)
...
def run_mlp_lm(config, train_data, dev_data):
    ...
    model = NPLM(tokenizer.vocab_size, config.embed_dim, config.local_window_size, config.hidden_dim, config.num_blocks, config.dropout_p)
