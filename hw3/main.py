...
def explore_mlp_structures(dev_d: Dict[str, List[Union[str, int]]],
    ...
    # iterate over hidden_dims to define train config, run the training and generate the plots
...
def explore_mlp_activations(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for activation, activation_name in zip(activations, activation_names):
        train_config = EasyDict({
            ...
            'save_path': f'model_activation_{activation_name}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': activation,  # non-linear activation function
        })
        ...
        all_emb_epoch_dev_accs.append(epoch_dev_accs)
        all_emb_epoch_dev_losses.append(epoch_dev_loss)
        visualize_epochs(epoch_train_losses, epoch_dev_loss, f"mlp_activation_{activation_name}_loss.png")
...
def explore_mlp_learning_rates(dev_d: Dict[str, List[Union[str, int]]],
    ...
    lrs = [0.002, 0.005, 0.01, 0.02, 0.05, 0.1]
    ...
    for lr, lr_name in zip(lrs, lrs_names):
        train_config = EasyDict({
            ...
            'num_epochs': num_epochs,  # the total number of times all the training data is iterated over
            'hidden_dims': hidden_dims,  # the number of neurons in each hidden layer
            'save_path': f'model_learning_rate_{lr_name}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': activation,  # non-linear activation function
        })
        ...
        all_emb_epoch_dev_accs.append(epoch_dev_accs)
        all_emb_epoch_dev_losses.append(epoch_dev_loss)
        visualize_epochs(epoch_train_losses, epoch_dev_loss, f"mlp_learning_rate_{lr_name}_loss.png")

    visualize_configs(all_emb_epoch_dev_accs, lrs_names, "Accuracy", "./all_mlp_learning_rate_acc.png")
    visualize_configs(all_emb_epoch_dev_losses, lrs_names, "Loss", "./all_mlp_learning_rate_loss.png")
