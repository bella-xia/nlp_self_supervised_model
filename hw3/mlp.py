    ...
    def __init__(self, embed_dim: int, num_classes: int, hidden_dims: List[int], activation: str = 'Sigmoid'):
        ...
        if activation == 'Sigmoid':
            ...
            if activation == 'Tanh':
                ...
                raise Exception("unrecognizable activation function")

        # linear layers for the MLP
        self.linears = nn.ModuleList()
        input_layer = embed_dim
        for out_layer in hidden_dims:
            self.linears.append(nn.Linear(input_layer, out_layer))
            input_layer = out_layer
        self.linears.append(nn.Linear(input_layer, num_classes))
