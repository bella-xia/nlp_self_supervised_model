...
def create_ngrams(data, n, splitter, tokenizer):
    ...
    for paragraph in tqdm(data['text']):
        ...
        for sentence in splitter.split(paragraph):
            ...
            for idx, _ in enumerate(tokens):
                ...
                single_ngram = tuple(tokens[idx:idx+n])
                single_ngram_context = tuple(tokens[idx:idx+n-1])
                ngrams[single_ngram] += 1
                ngram_context[single_ngram_context] += 1
                next_word_candidates[single_ngram_context].add(tokens[idx+n-1])
                #raise NotImplementedError
                # Your code ends here
    ...
    next_word_pred = {}
    next_word_scores = {}
    for context, next_words in tqdm(sorted_next_word_candidates.items()):
        ...
        for nw in next_words:
            ...
            context_and_nw = tuple(list(context) + [nw])
            ngram_counter = ngrams[context_and_nw]
            context_counter = ngram_context[context]
            prob = float(ngram_counter) / float(context_counter)
            scores.append(prob)
            #raise NotImplementedError
            # Your code ends here
...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    ...
    curr_scores = list(word_scores[context])
    curr_candidates = list(word_candidates[context])
    curr_zipped = list(zip(curr_scores, curr_candidates))
    curr_zipped.sort(key=lambda x: x[0], reverse=True)
    top_scores, top_candidates = zip(*curr_zipped)
    top_scores = top_scores[:top]
    top_candidates = top_candidates[:top]
