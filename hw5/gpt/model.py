...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # TODO: create a causal mask for attention matrix of shape [config.block_size, config.block_size]
        # (config.block_size is the maximum sequence length)
        #   The matrix should has 1s in the lower left triangular part (including the diagonal) and 0s in the
        # upper right.
        ...
        casual_mask = torch.tril(
            torch.ones(config.block_size, config.block_size), diagonal=0
        )
        # your code ends here
    ...
    def forward(self, x):
        ...
        att = q @ torch.transpose(k, 2, 3) / math.sqrt((C // self.n_head))
        ...
        boolean_mask = ~torch.gt(self.causal_mask[:, :, :T, :T], 0)
        att = att.masked_fill(boolean_mask, torch.tensor(-float("inf")))
    ...
    def forward(self, idx, targets=None, mask=None):
        ...
        if targets is not None:
            ...
            if mask is not None:
                ...
                assert 0 not in mask.sum(
                    dim=1
                ), "each sequence must have at least one token unmasked"
                loss = loss * mask
